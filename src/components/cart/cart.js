            ...
            {cartItems.length === 0 ? <h2 className={"mt-5"}>Your cart is empty</h2> : (
                <Fragment>
                    <div className="container container-fluid">
                        <h2 className="mt-5">Your
                            Cart: <b>{cartItems && cartItems.length} item{cartItems.length > 1 ? 's' : ''}</b></h2>
                        <div className="row d-flex justify-content-between">
                                        ...
                                        <div className="row">
                                            ...
                                            <div className="col-4 col-lg-2 mt-4 mt-lg-0">
                                                <div className="stockCounter d-inline">
                                                    Sub-total <span
                                                    className="ml-1 ">{item.quantity * item.price}</span>
                                                </div>
                                            </div>
                            ...
                            <div className="col-12 col-lg-3 my-4">
                                <div id="order_summary">
                                    ...
                                    <hr/>
                                    <p>Subtotal: <span
                                        className="order-summary-values">{cartItems.reduce((acc, item) => (acc + Number(item.quantity)), 0)} (Units)</span>
                                    </p>
                                    <p>Est. total: <span
                                        className="order-summary-values">{cartItems.reduce((acc, item) => acc + item.quantity * item.price, 0)}    </span>
                                    </p>
                                    <hr/>
                                    <button onClick={checkoutHandler} className="btn btn-primary btn-block">Check out</button>
                                </div>
                            </div>
                        </div>
                    </div>
